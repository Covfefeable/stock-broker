...
def batch_sync_etf_daily_history(
    user: User, *, task_id: str | None = None, log_result: bool = True
) -> dict:
    synced_etf_keys = (
        db.session.query(
    ...
    etfs_with_latest = (
        db.session.query(
            Etf,
            func.max(EtfDailyBar.trade_date).label("latest_trade_date"),
        )
        .join(
            synced_etf_keys,
            (Etf.exchange_code == synced_etf_keys.c.exchange_code)
            & (Etf.ticker == synced_etf_keys.c.ticker),
        )
        .join(
            EtfDailyBar,
            (Etf.exchange_code == EtfDailyBar.exchange_code)
            & (Etf.ticker == EtfDailyBar.ticker),
        )
        .group_by(Etf.id)
        .order_by(Etf.exchange_code.asc(), Etf.ticker.asc(), Etf.id.asc())
        .all()
    )
    if not etfs_with_latest:
        raise DataSyncError("当前没有已同步过历史日线的 ETF，无法执行批量补全。")

    target_end_date = beijing_today()
    pending_etfs = [
        (etf, latest_trade_date, target_end_date)
        for etf, latest_trade_date in etfs_with_latest
        if latest_trade_date is not None and latest_trade_date < target_end_date
    ]
    ...
    if not pending_etfs:
        if log_result:
            log_event(
                ...
                task_id=task_id,
                event_type="data_sync_batch",
                event_name="batch_sync_etf_daily_history",
                source="canghai",
                target=SYNC_ITEM_ETF_DAILY_HISTORY,
                status="success",
                level="info",
                message=f"批量自动补全完成，共扫描 {len(etfs_with_latest)} 只 ETF，均已追平北京时间今日，无需发起远程请求。",
                records_affected=0,
                started_at=datetime.now(timezone.utc),
                finished_at=datetime.now(timezone.utc),
                duration_ms=0,
        ...
        return {
            "status": "success",
            "totalEtfs": len(etfs_with_latest),
            "successCount": 0,
            "failedCount": 0,
            "skippedCount": len(etfs_with_latest),
            "recordsAffected": 0,
            "durationMs": 0,
        }
    ...
    started_at = datetime.now(timezone.utc)
    started_perf = perf_counter()
    total = len(etfs_with_latest)
    success_count = 0
    failed_count = 0
    skipped_count = total - len(pending_etfs)
    total_records = 0
    failed_examples: list[str] = []

    for etf, latest_trade_date, exchange_latest_date in pending_etfs:
        try:
            result = sync_etf_daily_history(
                user=user,
            ...
            failed_count += 1
            if len(failed_examples) < 5:
                failed_examples.append(f"{etf.exchange_code}/{etf.ticker}: {exc}")

    finished_at = datetime.now(timezone.utc)
    duration_ms = int((perf_counter() - started_perf) * 1000)
    message = (
        f"批量自动补全完成，共扫描 {total} 只 ETF，发起补全 {len(pending_etfs)} 只，成功 {success_count}，"
        f"失败 {failed_count}，其中 {skipped_count} 只 ETF 已追平北京时间今日，新增或更新 {total_records} 条日线记录。"
    )
    if failed_examples:
        message = f"{message} 失败示例：{'；'.join(failed_examples)}"
...
def batch_sync_stock_and_index_daily_history(user: User, *, task_id: str | None = None) -> dict:
    ...
    try:
        etf_result = batch_sync_etf_daily_history(user, task_id=task_id, log_result=False)
    except DataSyncError as exc:
        if "当前没有已同步过历史日线的 ETF" in str(exc):
            etf_result = {
                "status": "success",
                "totalEtfs": 0,
                "successCount": 0,
    ...
    if stock_result.get("totalStocks", 0) == 0 and etf_result.get("totalEtfs", 0) == 0 and index_result.get("totalIndexes", 0) == 0:
        raise DataSyncError("当前没有已同步过历史日线的股票、ETF或指数，无法执行批量补全。")

    message = (
        f"批量同步股票/ETF/指数日线完成：股票共 {stock_result.get('totalStocks', 0)} 只，"
        f"ETF 共 {etf_result.get('totalEtfs', 0)} 只，指数共 {index_result.get('totalIndexes', 0)} 个，"
        f"成功 {int(stock_result.get('successCount', 0)) + int(etf_result.get('successCount', 0)) + int(index_result.get('successCount', 0))}，"
        f"失败 {int(stock_result.get('failedCount', 0)) + int(etf_result.get('failedCount', 0)) + int(index_result.get('failedCount', 0))}，"
        f"跳过 {int(stock_result.get('skippedCount', 0)) + int(etf_result.get('skippedCount', 0)) + int(index_result.get('skippedCount', 0))}，"
        f"新增或更新 {int(stock_result.get('recordsAffected', 0)) + int(etf_result.get('recordsAffected', 0)) + int(index_result.get('recordsAffected', 0))} 条记录。"
    )
    log_event(
        user=user,
        task_id=task_id,
        ...
        status=(
            "success"
            if stock_result.get("status") == "success" and etf_result.get("status") == "success" and index_result.get("status") == "success"
        ...
        + int(etf_result.get("recordsAffected", 0))
    ...
    return {
        ...
        + int(etf_result.get("durationMs", 0))
