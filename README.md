...
![规则引擎](docs/images/rule-engine.png)

![回测实验室评估列表](docs/images/backtest-lab-evaluation.png)
...
- 交易日历。
- 股票历史送股/拆股信息。
- 已同步资产统计、覆盖度查询、行情数据浏览器，以及股票/ETF/指数日线批量补全。
...
股票历史日线同步会自动拉取送股/拆股信息。行情浏览、策略预览、Agent、回测实验室和交易决策应通过服务层统一取数；股票会应用拆股调整，ETF 与指数按各自日线表取数，避免不同模块各自处理导致口径漂移。

### 策略搭建

- 策略列表使用真实 API 数据，支持筛选、分页、排序、查看、编辑、归档、删除。
- 新建策略支持选择国家/地区、资产类型、股票/ETF/指数标的。
...
综合分数统一由后端服务计算，权重可在系统设置中调整。默认权重为：

```text
综合分数 = 年化收益 * 0.7 + Sharpe * 5 - 最大回撤 * 0.3
```

### AI Agent

AI Agent 用于在单一股票、ETF 或指数上自动迭代策略：
...
- 支持定时计划按日、周、月或分钟间隔触发 Agent，并记录计划执行历史。
...
![回测实验室区间详情](docs/images/backtest-lab-period-detail.png)
...
交易决策页用于在已有策略基础上查看当前标的的最新信号：

- 支持股票、ETF 和指数。
- 可选择一个主策略，并附加多个参考策略。
- 统一使用策略 DSL 与行情服务计算买卖条件，输出当前动作、命中规则和解释信息。
...
- `/scheduled-plans`：Agent 定时计划。
- `/backtest-lab`：回测实验室列表。
- `/backtest-lab/[id]`：评估详情。
- `/trade-decisions`：交易决策。
- `/settings`：系统设置。
...
- `scheduled_plans`：Agent 定时计划。
- `scheduled_plan_runs`：定时计划执行记录。
- `event_logs`：通用事件日志。
...
```bash
cd api
uv run pytest tests/services -q -s
```
